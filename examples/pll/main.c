...
static void clock_setup(struct clock_config_t *cfg) {
	...
	CGU_OSC = (cfg->ndiv << CGU_OSC_NDIV_SHIFT) | 0x707;
	CGU_CON0 = (cfg->k1 << CGU_CON0_PHASE1_K1_SHIFT) | (cfg->k1 << CGU_CON0_PHASE1_K2_SHIFT);
	CGU_CON1 = CGU_CON1_AHB_CLKSEL_PHASE1 | CGU_CON1_FSYS_CLKSEL_PLL;
	CGU_CON2 = (cfg->cpu_div << CGU_CON2_CPU_DIV_SHIFT) | CGU_CON2_CPU_DIV_EN;

	while (!(CGU_STAT & CGU_STAT_LOCK));
}
