...
static void test_registers(void) {
	...
	TPU_RFCON2 = expected;
	test_eq_u32("RFCON2 implemented fields", expected, TPU_RFCON2);
}
...
static void test_transfers(void) {
	...
	test_transfer("16-bit trailing-edge transfer", TPU_RFCON2_SSCBM_16 | TPU_RFCON2_SSCPB_TRAILING_EDGE, 0xA55A);
	test_transfer("RFSTR3 transfer", TPU_RFCON2_SSCBM_8 | TPU_RFCON2_SSCSB_RFSTR3, 0x005A);
	test_transfer("3.25 MHz transfer", TPU_RFCON2_SSCBM_16 | TPU_RFCON2_SSCFB_3_25MHZ, 0x5AA5);
}

static void test_continuous_transfer(void) {
	test_category("RF SSC continuous transfer");
	rf_ssc_tcm_init();
	TPU_RFCON1 = 0;
	TPU_RFSSC_SRC = MOD_SRC_SRE | MOD_SRC_CLRR;
	bool completed = rf_ssc_tcm_transfer(
		TPU_RFCON2_SSCBM_12 | TPU_RFCON2_SSCFB_3_25MHZ | TPU_RFCON2_SSCEN,
		0x0868, 0x000E, true
	);

	test_check("two 12-bit words form one continuous transfer", completed);
}
