...
sub build_shard {
	...
	die "Invalid instruction in DSP shard $shard\n" if read_file($dis_path) =~ /\[ERROR\]/;
...
my $builder_fingerprint = sha256_hex(
	read_file(__FILE__),
	read_file($teak_assembler),
	read_file($teak_disassembler),
);
my @pending;
my $cached = 0;
...
if (@failed) {
	for my $failure (@failed) {
		...
		print STDERR "DSP shard $shard failed with status $status:\n", read_file($log_path);
	}
	remove_tree($temp_dir);
	die scalar(@failed) . " DSP shard(s) failed\n";
}

remove_tree($temp_dir);
printf "DSP images: %u cached, %u rebuilt with %u worker(s)\n", $cached, $shard_count - $cached, $jobs;
