...
static bool load_dsp_image(
	...
	while (dsp_container_next(&reader, &record)) {
		...
		if (!load_words(record.command, record.destination, record.data, record.words,
			first_word, second_word, page_word, patch_words))
		{
			return false;
		}
	}
...
static bool load_probe_image(uint16_t first_word, uint16_t second_word) {
	...
	return load_dsp_image(DSP_EXPANSION_PROBE_IMAGE, sizeof(DSP_EXPANSION_PROBE_IMAGE),
		first_word, second_word, page_word, true);
#else
	return load_dsp_image(DSP_OPCODE_PROBE_IMAGE, sizeof(DSP_OPCODE_PROBE_IMAGE),
		first_word, second_word, DSP_PROBE_PAGE_ZERO_WORD, true);
#endif
}
...
int main(void) {
	...
	for (uint32_t opcode = DSP_OPCODE_PROBE_FIRST; opcode <= DSP_OPCODE_PROBE_LAST; opcode++) {
		...
		if (!load_probe_image((uint16_t) opcode, 0))
			return fail_transport("DSP image load failed during opcode probe", (uint16_t) opcode);
		if (!branch_to_probe())
...
int main(void) {
	...
	if (!load_dsp_image(DSP_PROGRAM_BACKING_IMAGE, sizeof(DSP_PROGRAM_BACKING_IMAGE), 0, 0, 0, false))
		...
		family_index <= DSP_EXPANSION_PROBE_FAMILY_LAST; family_index++)
	{
			...
			expansion <= DSP_EXPANSION_PROBE_WORD_LAST; expansion++)
		{
			...
			if (!load_probe_image(family->first_word, (uint16_t) expansion))
				return fail_expansion_transport("DSP image load failed during expansion probe", family_index,
					(uint16_t) expansion);
