...
sub build_shard {
	...
	# Teakra is not the oracle and may reject a valid noncanonical opcode in its
	# textual disassembly. A nonzero reader exit still rejects a malformed DSP1.
...
my $builder_fingerprint = sha256_hex(read_file(__FILE__), read_file($makedsp1), read_file($dsp1_reader));
my @pending;
my $cached = 0;
...
if (@failed) {
	for my $failure (@failed) {
		...
		print STDERR "DSP1 alias shard $shard failed with status $status:\n", read_file($log_path);
	}
	remove_tree($temp_dir);
	die scalar(@failed) . " DSP1 alias shard(s) failed\n";
}

remove_tree($temp_dir);
printf "DSP1 alias images: %u cached, %u rebuilt with %u worker(s)\n", $cached, $shard_count - $cached, $jobs;
